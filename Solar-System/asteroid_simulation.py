...
nbodies = 11  # Number of bodies (Sun + 9 planets including "My Planet" + 1 asteroid)
...
mura = 8.681e25
mnep = 1.024e26
mast = 0.95e21  # Mass of the asteroid
...
mass = [msun, mearth, mr, mmer, mven, mmar, mjup, msat, mura, mnep, mast]
...
x = [0, AU, r, 0.4 * AU, 0.7 * AU, AU * 1.5, AU * 5.2, AU * 9.5, AU * 19.8, AU * 30, AU * 4.7]  # Asteroid at 4.7 AU
...
tmax = 86400 * 366 * 30  # Simulate for 30 Earth years to capture asteroid dynamics
...
for t in np.arange(dt, tmax, dt):
    ...
    for n1 in range(1, len(mass), 1):
        ...
        KE = 0.5 * mast * (vy[i])**2  # Calculate kinetic energy of the asteroid (though i is undefined here)
        print(KE)  # Print kinetic energy
    ...
    for n1 in range(1, len(mass), 1):
        if x[n1] > 0 and y[n1] * yold[n1] < 0:  # Check if body crosses x-axis
            ...
            print(n1, t_orbit[n1], t_orbit[n1] / 3600 / 24)  # Print body index, orbital period, and period in days

# Plot the orbits of all bodies (excluding the Sun)
for n1 in range(1, nbodies):
    plt.plot(xlist[n1], ylist[n1], label='id %s' % n)  # Plot orbit for each body (n is undefined here)
plt.savefig('test fig')  # Save plot as 'test fig'